...
Процедура УстановитьВКаталог(Знач Каталог, Знач УдалятьФайлыИзИндексаГит = Ложь) Экспорт
	...
	Если НЕ ЭтоWindows() Тогда
		CRLF2LF(КаталогХуков,ОбъединитьПути(КаталогХуков, "pre-commit"));
...
Процедура CRLF2LF(РабочийКаталог, ИмяФайла)

	Параметры = Новый Массив;
	Параметры.Добавить(" $'s/\r$//'");
	Параметры.Добавить("-i");
	Параметры.Добавить(ИмяФайла);

	Команда = Новый Команда;

	Команда.УстановитьКоманду("sed");
	Команда.УстановитьКодировкуВывода(КодировкаТекста.UTF8);
    Команда.УстановитьРабочийКаталог(РабочийКаталог);
	Команда.ДобавитьПараметры(Параметры);

	Лог.Отладка("Заменяем CRLF на LF для файла %1", ИмяФайла);
