...
Функция ЗапускВКоманднойСтроке()
	...
	Лог_cmdline.УстановитьУровень(УровниЛога.Информация);
	...
	Попытка
		...
		Если НЕ ПустаяСтрока(Аргументы.ЗначенияПараметров["--db-connection-string"]) Тогда
			КонтекстКонфигуратора = Новый Структура("КлючСоединенияСБазой, ИмяПользователя, Пароль");
			КонтекстКонфигуратора.КлючСоединенияСБазой = Аргументы.ЗначенияПараметров["--db-connection-string"];
		...
		ИначеЕсли Команда = ВозможныеКоманды().ОбработатьИзмененияИзГит Тогда
			ОбработатьИзмененияИзГит(
				...
				Аргументы.ЗначенияПараметров["--use-bin-folder"]
			);
...
Процедура ДобавитьОбщиеПараметры(Знач Парсер)
	Парсер.ДобавитьИменованныйПараметр("--db-connection-string", "Строка подключения к БД", Истина);
...
Процедура ДобавитьОписаниеКомандыИзмененияПоЖурналуГит(Знач Парсер)
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды, "--use-bin-folder");
	Парсер.ДобавитьКоманду(ОписаниеКоманды);
...
Процедура ВывестиСправку()
	...
	Сообщить("	--git-precommit outputPath [--remove-orig-bin-files] [--use-bin-folder]");
	...
	Сообщить("		Если передан флаг --use-bin-folder, обрабатываются только файлы в папке bin в корне репозитория");
	...
	Сообщить("	--db-connection-string строка подключения к базе данных");
...
Процедура ОбработатьИзмененияИзГит(Знач ВыходнойКаталог, Знач УдалятьФайлыИзИндексаГит, Знач ИспользоватьBin) Экспорт
	...
	Если ИспользоватьBin Тогда
		КореньРепо = ОбъединитьПути(КореньРепо, "bin");
	КонецЕсли;
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		Если ИспользоватьBin Тогда
			Если Лев(ИмяФайла, 3) = "bin" Тогда
				ИмяФайла = Сред(ИмяФайла, 5);
			Иначе
				Лог.Отладка("Используется bin и файл не принадлежит этой папке");
				Продолжить;
			КонецЕсли;
		КонецЕсли;
